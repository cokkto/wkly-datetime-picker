...
export class WklyDateTimePickerOverlayService {
  constructor(
    ...
    private traps: FocusTrapFactory,
  ) {}
  open(
    trigger: HTMLElement,
    ...
    overlay.overlayElement.setAttribute("aria-modal", "true");
    overlay.overlayElement.setAttribute("aria-label", "Date and time picker");
    overlay.overlayElement.style.overflow = "auto";
    const trap = this.traps.create(overlay.overlayElement);
    const sub = overlay.backdropClick().subscribe(() => {
      if (backdrop) onCancel("backdrop");
    });
    const key = overlay.keydownEvents().subscribe((event) => {
      if (event.key === "Escape") {
...
@Directive({
  selector: "[wklyDateTimePickerOverlay]",
  exportAs: "wklyOverlay",
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => WklyDateTimePickerOverlayDirective),
      multi: true,
    },
    {
...
@NgModule({
  imports: [OverlayModule, WklyDateTimePickerModule],
  declarations: [WklyDateTimePickerOverlayDirective],
  exports: [WklyDateTimePickerOverlayDirective],
})
