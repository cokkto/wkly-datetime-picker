...
@Component({
  ...
  template: `<div class="field" (wheel)="wheel($event)" (touchstart)="touchStart($event)" (touchend)="touchEnd($event)">
    ...
    <input [attr.aria-label]="label" [attr.aria-invalid]="invalid" [disabled]="disabled" [value]="display" [attr.inputmode]="labels ? 'text' : 'numeric'" autocomplete="off" (focus)="begin($event)" (input)="type($event)" (blur)="finish()" (keydown)="key($event)" />
...
export class WklyFieldComponent implements OnDestroy {
  ...
  get display(): string { if (this.typed !== null) return this.typed; return this.value === null ? '' : this.labels ? this.labels[this.value - this.min] || String(this.value) : new Intl.NumberFormat(this.locale, { useGrouping: false, minimumIntegerDigits: this.max < 60 ? 2 : 1 }).format(this.value); }
  ...
  stepBy(delta: number): void { if (this.disabled) return; this.cancelInput(); this.typed = null; this.value = this.next(delta); this.valueChange.emit(this.value); this.complete.emit(); }
  ...
  type(event: Event): void {
    ...
    if (numeric) {
      ...
      this.typed = raw;
      const matches = this.labels.map((label, index) => ({ label, index })).filter(item => item.label.toLocaleLowerCase(this.locale).includes(search));
    ...
    this.inputTimer = setTimeout(() => { this.inputTimer = null; if (this.labels) this.typed = null; if (!this.disabled) this.complete.emit(); }, 1000);
  }
  private cancelInput(): void { if (this.inputTimer !== null) clearTimeout(this.inputTimer); this.inputTimer = null; }
  finish(): void { this.cancelInput(); this.typed = null; this.complete.emit(); }
  key(event: KeyboardEvent): void {
    if (event.key === 'Escape') { event.preventDefault(); event.stopPropagation(); this.cancelInput(); this.typed = null; this.value = this.before; this.valueChange.emit(this.before); (event.target as HTMLInputElement).value = this.display; (event.target as HTMLInputElement).blur(); }
    if (event.key === 'Enter') { event.preventDefault(); event.stopPropagation(); this.finish(); }
  ...
  wheel(event: WheelEvent): void { if (this.disabled || Math.abs(event.deltaY) < 1) return; event.preventDefault(); this.cancelInput(); this.typed = null; this.value = this.next(event.deltaY > 0 ? 1 : -1); this.valueChange.emit(this.value); if (this.wheelTimer !== null) clearTimeout(this.wheelTimer); this.wheelTimer = setTimeout(() => { this.wheelTimer = null; this.complete.emit(); }, 140); }
  ngOnDestroy(): void { this.cancelInput(); if (this.wheelTimer !== null) clearTimeout(this.wheelTimer); }
