...
async function topRowOffset(scroller: Locator): Promise<number> {
  return scroller.evaluate((element: HTMLElement) => {
    ...
    return Math.min(...rows.map(row => Math.abs(row.getBoundingClientRect().top - top)));
  });
}
...
test("week buttons advance one row after mouse-wheel scrolling", async ({
  ...
  await expect.poll(async () => topRowOffset(panel.locator(".week-scroll"))).toBeLessThan(1);
...
test("mouse-wheel scrolling settles with a complete week at the top", async ({ page }) => {
  await page.goto("/virtualization");
  const panel = page.getByTestId("virtual-weeks");
  ...
  await expect.poll(async () => (await visibleWeek(panel)).first)
  ...
  expect(afterWeek.topDay - beforeWeek.topDay).toBe(7 * (afterWeek.first - beforeWeek.first));
});

test("enlarged calendar rows also snap to a complete week", async ({ page }) => {
...
test("touch scrolling settles with a complete week at the top", async ({ page }, info) => {
  ...
  try {
    ...
    for (let i = 1; i <= 6; i++) {
      await session.send("Input.dispatchTouchEvent", {
        type: "touchMove", touchPoints: [{ x, y: y - i * 24 }],
      });
    }
...
test("wheel deltas leave three buffer rows above the first visible week", async ({ page }) => {
  await page.goto("/virtualization");
  const panel = page.getByTestId("virtual-weeks");
  const scroller = panel.locator(".week-scroll");
  ...
  for (const delta of [12, 26, 51, 80, 121, -13, -29, -53, -118, 37]) {
    ...
    const state = await scroller.evaluate((element: HTMLElement) => {
      ...
      const annotation = firstVisible?.querySelector<HTMLElement>(".annotation");
      return {
    ...
    expect(state.annotationBottom!, `delta ${delta}`).toBeGreaterThan(state.viewportTop);
    const reported = await visibleWeek(panel);
    expect(reported.topDay - initial.topDay, `delta ${delta}`)
      .toBe(7 * (reported.first - initial.first));
  }
});

test("scrollend navigates to the first absolute week already reported", async ({ page }) => {
  await page.goto("/virtualization");
  const panel = page.getByTestId("virtual-weeks");
  const scroller = panel.locator(".week-scroll");
  const initial = await visibleWeek(panel);
  await scroller.hover();
  await page.mouse.wheel(0, 120);
  await expect.poll(async () => (await visibleWeek(panel)).first).toBeGreaterThan(initial.first);
  const reported = await visibleWeek(panel);
  await scroller.dispatchEvent("scrollend");
  await expect.poll(async () => (await visibleWeek(panel)).first).toBe(reported.first);
  await expect.poll(async () => topRowOffset(scroller)).toBeLessThan(1);
});
