...
(async () => {
  ...
  Component({
    ...
    standalone: false,
    template:
      '<wkly-datetime-picker mode="date" value="1969-12-31T00:00:00.000Z"></wkly-datetime-picker>',
  })(Host);
  class App {}
  NgModule({
    imports: [
      BrowserModule.withServerTransition
        ? BrowserModule.withServerTransition({ appId: "wkly-ssr" })
        : BrowserModule,
      ServerModule,
      base.WklyDateTimePickerModule,
    ],
    declarations: [Host],
    bootstrap: [Host],
    providers: [
      {
        provide: base.WKLY_CLOCK,
        useValue: {
          now: () => {
            throw new Error("SSR must not read the clock");
          },
        },
      },
    ],
  ...
  const html = await renderModule(App, {
    document: "<html><body><server-host></server-host></body></html>",
    url: "/",
  });
  assert(html.includes("1969"));
  assert(html.includes('role="grid"'));
  console.log(
    "PASS packed shared imports and Angular server rendering without clock access",
  );
})().catch((error) => {
  console.error(error);
  process.exitCode = 1;
