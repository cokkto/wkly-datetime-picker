...
function rows(metadata = supported) {
  ...
  for (const entry of fs.readdirSync(path.join(root, 'projects'))) {
    if (/^wkly-datetime-picker\.\d+$/.test(entry) && !metadata[entry.split('.').pop()])
...
function affected(files, metadata = supported) {
  ...
  if (!files) return all;
  const packages = fs.readdirSync(path.join(root, 'projects')).filter(name => fs.existsSync(path.join(root, 'projects', name, 'package.json')));
  const manifests = Object.fromEntries(packages.map(name => [name, read(path.join(root, 'projects', name, 'package.json'))]));
  ...
  do {
    ...
    for (const [name, pkg] of Object.entries(manifests)) {
      if (!changed.has(name) && Object.keys({ ...pkg.dependencies, ...pkg.peerDependencies, ...pkg.devDependencies }).some(dep => changed.has(dep))) {
        changed.add(name); grew = true;
      }
    }
  } while (grew);
  return all.filter(row => changed.has(metadata[row.angular].package)
    || files.some(file => file.startsWith(`projects/wkly-datetime-picker.runtime.${row.angular}/`)));
}

function prepare(major) {
  if (!rows().some(row => row.angular === major)) throw new Error(`Unsupported Angular ${major}`);
  ...
  for (const name of ['projects', 'scripts', 'tests', 'docs', 'tsconfig.json', 'supported-angular.json', 'LICENSE', 'README.md']) {
    fs.cpSync(path.join(root, name), path.join(dir, name), { recursive: true, filter: source => !['node_modules', 'e2e'].includes(path.basename(source)) });
  }
  write(path.join(dir, 'package.json'), { name: 'wkly-compatibility', private: true, dependencies: supported[major].dependencies });
  fs.writeFileSync(path.join(dir, '.npmrc'), 'package-lock=true\nstrict-peer-deps=true\naudit=false\nfund=false\n');
  return dir;
}

function run(command, args, cwd, capture = false) {
  return execFileSync(command, args, { cwd, stdio: capture ? 'pipe' : 'inherit', encoding: 'utf8', shell: process.platform === 'win32' && command === 'npm', env: { ...process.env, NG_CLI_ANALYTICS: 'false' } });
}

function test(major) {
  if (!supported[major]) throw new Error(`Unsupported Angular ${major}`);
  const dir = path.join(root, '.compat', major);
  if (!fs.existsSync(path.join(dir, 'package.json'))) throw new Error(`Run node scripts/compatibility.cjs prepare ${major} with Node 22+ first`);
  if (process.versions.node.split('.')[0] !== supported[major].node.split('.')[0]) throw new Error(`Use Node ${supported[major].node} for Angular ${major}`);
  ...
  for (const name of fs.readdirSync(path.join(dir, 'dist'))) {
    ...
    const packed = JSON.parse(run('npm', ['pack', '--json'], packageDir, true))[0];
    dependencies[name] = `file:../dist/${name}/${packed.filename}`;
  }
  write(path.join(consumer, 'package.json'), { name: 'wkly-consumer', private: true, dependencies });
...
function matrix() {
  ...
  const base = event.pull_request?.base.sha || (process.env.GITHUB_EVENT_NAME === 'push' ? event.before : undefined);
  if (base && !/^0+$/.test(base)) {
    const head = event.pull_request?.head.sha || process.env.GITHUB_SHA || 'HEAD';
    const range = event.pull_request ? `${base}...${head}` : `${base}..${head}`;
    files = run('git', ['diff', '--name-only', '--no-renames', '-z', range], root, true).split('\0').filter(Boolean);
  }
...
if (require.main === module) {
  ...
  else throw new Error('Usage: compatibility.cjs matrix | prepare <major> | test <major>');
}
