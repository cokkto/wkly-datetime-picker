...
(async () => {
  assets();
  const contexts = await Promise.all(builds.map(async ({ esbuild, options }) => {
  ...
  const server = http.createServer((req, res) => {
    ...
    let file = requested;
    if (pathname === '/' || (!fs.existsSync(file) && !pathname.startsWith('/runtime/'))) file = path.join(output, 'index.html');
  ...
  server.listen(4200, '127.0.0.1', () => console.log(`WKLY evergreen showcase: http://127.0.0.1:4200 (calendar runtimes: ${majors.join(', ')})`));
