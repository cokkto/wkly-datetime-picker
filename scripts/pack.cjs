...
for (const name of [
  ...
  if (!angularPackages.includes(name)) {
    for (const dep of [
      ...
      if (
        ...
        dep === "rxjs" ||
        /^wkly-datetime-picker\.\d+$/.test(dep)
      )
        throw new Error(
          "Framework dependency leaked into shared package: " + dep,
        );
  }
  for (const dep of Object.keys(pkg.dependencies || {}))
    if (/hebcal|material|playwright|showcase/.test(dep))
  ...
  for (const file of result[0].files)
    if (/showcase|e2e|snapshot|material|hebcal/.test(file.path))
      throw new Error("Forbidden package file: " + file.path);
  if (name === "wkly-datetime-picker")
    for (const file of result[0].files)
      if (/component|ngfactory|\.metadata\.json|cdk-overlay/.test(file.path))
        throw new Error(
          "Angular artifact leaked into shared presentation: " + file.path,
        );
  console.log(
    name +
      ": " +
      result[0].files.length +
      " package files; boundaries verified",
  );
}
